        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                if row:
                    ...
                    # Map pds_extracted_data to pds_data for frontend compatibility
                    if candidate.get('pds_extracted_data'):
                        candidate['pds_data'] = candidate['pds_extracted_data']
                    elif not candidate.get('pds_data'):
                        candidate['pds_data'] = {}
                    ...
                    # Enhance personal_info in pds_data if it exists
                    if candidate.get('pds_data') and isinstance(candidate['pds_data'], dict):
                        if 'personal_info' in candidate['pds_data']:
                            personal_info = candidate['pds_data']['personal_info']
                            
                            # Clean and create full_name if it doesn't exist or needs cleaning
                            if personal_info.get('full_name'):
                                # Clean existing full_name (remove trailing "N/a" or similar)
                                full_name = personal_info['full_name']
                                full_name = re.sub(r'\s+N/a$', '', full_name, flags=re.IGNORECASE)
                                full_name = re.sub(r'\s+None$', '', full_name, flags=re.IGNORECASE)
                                full_name = full_name.strip()
                                if full_name:
                                    personal_info['full_name'] = full_name
                                else:
                                    personal_info['full_name'] = None
                            
                            # Create full_name if it doesn't exist or is now empty
                            if not personal_info.get('full_name'):
                                name_parts = [
                                    personal_info.get('first_name'),
                                    personal_info.get('middle_name'), 
                                    personal_info.get('surname'),
                                    personal_info.get('name_extension')
                                ]
                                valid_parts = [part.strip() for part in name_parts 
                                             if part and part.strip() and 
                                             part.strip().lower() not in ['n/a', 'none', 'null']]
                                if valid_parts:
                                    personal_info['full_name'] = ' '.join(valid_parts)
    ...
    def get_assessment_analytics(self, job_id: int = None, position_type_id: int = None) -> Dict:
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                filtered_where_conditions = base_where.copy()
                filtered_where_conditions.append("assessment_status != 'incomplete'")
                filtered_where = "WHERE " + " AND ".join(filtered_where_conditions)
                
                complete_where_conditions = base_where.copy()
                complete_where_conditions.append("assessment_status = 'complete'")
                complete_where = "WHERE " + " AND ".join(complete_where_conditions)
                ...
                # Score distribution - parameters need to be adjusted for filtered query
                filtered_params = params + ['incomplete']
                cursor.execute(f"""
                    SELECT 
                        CASE 
                ...
                # Recommendation distribution - parameters need to be adjusted for complete query
                complete_params = params + ['complete']
                cursor.execute(f"""
