    ...
    def get_assessment_analytics(self, job_id: int = None, position_type_id: int = None) -> Dict:
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cursor:
                ...
                # Build where clauses for different scenarios
                if base_where:
                    where_clause = "WHERE " + " AND ".join(base_where)
                    filtered_where = "WHERE " + " AND ".join(base_where + ["assessment_status != %s"])
                    complete_where = "WHERE " + " AND ".join(base_where + ["assessment_status = %s"])
                    
                    filtered_params = params + ['incomplete']
                    complete_params = params + ['complete']
                else:
                    where_clause = ""
                    filtered_where = "WHERE assessment_status != %s"
                    complete_where = "WHERE assessment_status = %s"
                    
                    filtered_params = ['incomplete']
                    complete_params = ['complete']
                ...
                """, complete_params)                recommendation_stats = {row['recommendation']: row['count'] for row in cursor.fetchall()}
