...
const CandidatesModule = {
    ...
    renderSemanticAnalysisResults(semanticData) {
        ...
        const overallScore = analysis.overall_relevance_score || analysis.overall_score || 0;
        const educationRelevance = analysis.education_relevance || 0;
        const experienceRelevance = analysis.experience_relevance || 0;
        const skillsRelevance = analysis.skills_relevance || analysis.training_relevance || 0;
                    ...
                    <div class="relevance-categories">
                        ...
                        <div class="relevance-category">
                            <div class="category-header">
                                ...
                                <span class="category-score">${(skillsRelevance).toFixed(1)}%</span>
                            </div>
                            <div class="category-bar">
                                <div class="bar-fill skills" style="width: ${skillsRelevance}%"></div>
                            </div>
                            <div class="category-insights">
    ...
    async populateAssessmentComparison(candidate) {
        ...
        try {
            const jobId = this.getJobIdForCandidate(candidate);
            if (jobId) {
                // Use the same endpoint as hybrid scoring since it includes all assessment data
                const response = await fetch(`/api/candidates/${candidate.id}/assessment/${jobId}`);
                if (response.ok) {
                    const result = await response.json();
                    if (result.success && result.assessment) {
                        // Transform the hybrid assessment data into comparison format
                        const comparisonData = {
                            traditional_assessment: result.assessment.university_assessment,
                            semantic_assessment: result.assessment.semantic_analysis,
                            hybrid_score: result.assessment.hybrid_score,
                            recommendations: result.assessment.recommendations || []
                        };
                        this.renderAssessmentComparisonResults(comparisonData);
                    } else {
                ...
                this.renderAssessmentComparisonError('No job selected for comparison');
            }
            }
        } catch (error) {
