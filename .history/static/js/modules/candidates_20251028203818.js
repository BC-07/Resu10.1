...
const CandidatesModule = {
    ...
    async showCandidateDetails(candidateId) {
        ...
        try {
            ...
            if (data.success) {
                ...
                console.error('Failed to load candidate:', candidateId, data.error || 'Unknown error');
                // Check if this is a 404 error (candidate not found)
                if (data.error && (data.error.includes('404') || data.error.includes('not found'))) {
                    ToastUtils.showError(`Candidate ${candidateId} not found. This candidate may have been removed from the system.`);
                    // Refresh candidates list after a short delay
                    setTimeout(() => {
                        this.loadCandidates();
                    }, 2000);
                } else {
            ...
            // Handle network/API errors, including 404s
    ...
    renderAssessmentResults(candidate, assessmentData) {
        ...
        // Handle both legacy and new hybrid assessment data structures
        const isHybrid = assessmentData.assessment_type === 'hybrid';
        ...
        // Get totals - hybrid vs legacy
        const universityTotal = assessmentData.university_total || assessmentData.automated_total || 0;
        const enhancedTotal = assessmentData.enhanced_total || assessmentData.overall_total || 0;
        const overallTotal = assessmentData.overall_total || 0;
        
        // Get semantic scores if available
        const semanticScores = assessmentData.semantic_scores || {};
        const hasSemanticAnalysis = assessmentData.has_semantic_analysis || isHybrid;
        
        // Get university criteria breakdown if available
        const universityCriteria = assessmentData.university_criteria_breakdown || {};
        
        let assessmentHTML = '';
        
        if (isHybrid && hasSemanticAnalysis) {
            // Render hybrid assessment view
            assessmentHTML = `
                <div class="assessment-overview hybrid-assessment">
                    <div class="assessment-type-badge">
                        <span class="badge badge-primary">🚀 Hybrid Assessment</span>
                        <small>University Criteria + AI Enhancement</small>
                    </div>
                    
                    <div class="assessment-scores-row">
                        <div class="score-section">
                            <div class="score-circle-large ${this.getScoreColorClass(universityTotal)}">
                                <span class="score-value">${universityTotal.toFixed(1)}</span>
                                <span class="score-label">University</span>
                            </div>
                            <p class="score-description">Official Criteria</p>
                        </div>
                        <div class="score-section">
                            <div class="score-circle-large ${this.getScoreColorClass(enhancedTotal)}">
                                <span class="score-value">${enhancedTotal.toFixed(1)}</span>
                                <span class="score-label">Enhanced</span>
                            </div>
                            <p class="score-description">With AI Analysis</p>
                        </div>
                    </div>
                </div>
                
                <div class="assessment-breakdown hybrid-breakdown">
                    <div class="criteria-tabs">
                        <button class="tab-btn active" data-tab="university">🏛️ University Criteria</button>
                        <button class="tab-btn" data-tab="semantic">🧠 Semantic Analysis</button>
                        <button class="tab-btn" data-tab="comparison">📊 Comparison</button>
                    </div>
                    
                    <div class="tab-content active" id="university-tab">
                        <div class="criteria-header">
                            <h6>Official University Assessment</h6>
                            <small class="text-muted">Based on LSPU Standards</small>
                        </div>
                        
                        <div class="criteria-list">
                            ${this.renderUniversityCriteriaList(breakdown, universityCriteria)}
                        </div>
                    </div>
                    
                    <div class="tab-content" id="semantic-tab">
                        <div class="criteria-header">
                            <h6>AI-Powered Semantic Analysis</h6>
                            <small class="text-muted">Job Relevance Intelligence</small>
                        </div>
                        
                        <div class="semantic-analysis">
                            ${this.renderSemanticAnalysis(semanticScores)}
                        </div>
                    </div>
                    
                    <div class="tab-content" id="comparison-tab">
                        <div class="criteria-header">
                            <h6>Assessment Comparison</h6>
                            <small class="text-muted">University vs Enhanced Scoring</small>
                        </div>
                        
                        <div class="comparison-chart">
                            ${this.renderAssessmentComparison(universityTotal, enhancedTotal, semanticScores)}
                        </div>
                    </div>
                </div>
            `;
        } else {
            // Render legacy assessment view (backward compatibility)
            ...
            assessmentHTML = `
                ...
                <div class="assessment-breakdown">
                    ...
                    <div class="criteria-list">
                        ${this.renderLegacyCriteriaList(breakdown)}
                    </div>
                </div>
            `;
        }
        ...
        // Add tab functionality for hybrid assessments
        if (isHybrid) {
            this.initializeAssessmentTabs();
        }
        
        // Add potential score editing functionality
        this.initializePotentialScoreEditor(candidate.id, breakdown.potential); 
