...
class EnhancedUniversityAssessmentEngine(UniversityAssessmentEngine):
    ...
    def assess_candidate_enhanced(self, candidate_data: Dict, job_data: Dict, 
        ...
        if include_traditional:
            try:
                # Map PDS fields to assessment engine expected format
                ...
                # Extract score - try multiple possible score fields
                traditional_score = (
                    traditional_result.get('final_score') or 
                    traditional_result.get('percentage_score') or 
                    traditional_result.get('automated_score') or 
                    0
                )
                
                result['traditional_score'] = traditional_score
                result['traditional_breakdown'] = {
                    'education': traditional_result.get('education_score', 0),
                    'experience': traditional_result.get('experience_score', 0), 
                    'training': traditional_result.get('training_score', 0),
                    'eligibility': traditional_result.get('eligibility_score', 0),
                    'performance': traditional_result.get('performance_score', 0),
                    'potential': traditional_result.get('potential_score', 0)
                }
    ...
    def _map_pds_fields_for_traditional_assessment(self, pds_data: Dict) -> Dict:
        """
        Map PDS fields to the format expected by traditional assessment engine
        
        Args:
            pds_data: Raw PDS data with fields like 'educational_background'
            
        Returns:
            Mapped data with fields like 'education_data' that traditional engine expects
        """
    ...
    def _calculate_university_criteria_score(self, candidate_data: Dict, job_data: Dict, manual_scores: Dict = None) -> Dict:
        """
        Calculate official university criteria score following the scoring sheet format
        
        Args:
            candidate_data: Candidate information with PDS data
            job_data: Job posting information  
            manual_scores: Manual scores for potential and performance (provided by user)
            
        Returns:
            Dictionary with official university scoring breakdown
        """
    ...
    def _calculate_university_education_score(self, candidate_data: Dict) -> float:
        ...
        for edu in educational_background:
            ...
            degree_course = edu.get('degree_course', '').lower()
