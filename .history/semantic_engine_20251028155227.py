...
class UniversitySemanticEngine:
    ...
    def is_available(self) -> bool:
        ...
        return SEMANTIC_DEPENDENCIES_AVAILABLE and self.model is not None
    ...
    def encode_candidate_profile(self, candidate_data: Dict) -> Optional[np.ndarray]:
        """
        Encode candidate profile into embedding vector
        ...
        try:
            ...
            if education and isinstance(education, list):
                for edu in education[:3]:  # Top 3 education entries
                    if isinstance(edu, dict):
                        ...
                        if degree or school:
                            profile_parts.append(f"Education: {degree} from {school}")
                ...
                for exp in experience[:3]:  # Top 3 work experiences
                    if isinstance(exp, dict):
                        ...
                        if position or company:
                            ...
                            if description:
                                exp_text += f" - {description[:100]}"  # Limit description
            ...
            # PDS Personal Info (if available)
            ...
            if pds_data and isinstance(pds_data, dict):
                ...
                if personal_info:
                    ...
                    if personal_info.get('civil_status'):
                        profile_parts.append(f"Status: {personal_info['civil_status']}")
    ...
    def calculate_detailed_semantic_score(self, candidate_data: Dict, job_data: Dict) -> Dict:
        ...
        try:
            ...
            skills_score = self._calculate_skills_relevance(candidate_data, job_data)
            
            return {
                'overall_score': round(overall_score, 3),
                'education_relevance': round(education_score, 3),
                'experience_relevance': round(experience_score, 3),
                'skills_relevance': round(skills_score, 3),
    ...
    def _calculate_education_relevance(self, candidate_data: Dict, job_data: Dict) -> float:
        """Calculate education-specific relevance"""
        try:
            ...
            for edu in education[:2]:  # Top 2 education entries
                if isinstance(edu, dict):
                    ...
                    if degree or school or major:
                        edu_text = f"{degree} {major} from {school}".strip()
    ...
    def _calculate_experience_relevance(self, candidate_data: Dict, job_data: Dict) -> float:
        """Calculate experience-specific relevance"""
        try:
    ...
    def _calculate_skills_relevance(self, candidate_data: Dict, job_data: Dict) -> float:
        """Calculate skills-specific relevance"""
        try:
            # Extract skills text
            skills = candidate_data.get('skills', [])
            if not skills:
            ...
            # Calculate similarity
            skills_embedding = self.encode_text(skills_text, "skills")
            job_embedding = self.encode_text(job_text, "job_skills_comparison")
            
            if skills_embedding is None or job_embedding is None:
                return 0.0
            
            return self.calculate_semantic_similarity(skills_embedding, job_embedding)
            
        except Exception as e:
            logger.error(f"Failed to calculate skills relevance: {e}")
            return 0.0
