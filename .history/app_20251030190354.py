        ...
        try:
            ...
            comparison_data = {
                'traditional_assessment': {
                    'total_score': round(university_total, 2),
                    'method': 'Basic qualification matching',
                    'factors': ['Education', 'Experience', 'Skills']
                },
                'enhanced_assessment': {
                    'hybrid_score': round(university_total, 2),
                    'semantic_score': round(semantic_scores.get('overall_score', 0) * 100, 2),
                    'combined_score': round((university_total + (semantic_scores.get('overall_score', 0) * 100)) / 2, 2),
                    'method': 'University criteria + AI semantic analysis',
                    'factors': ['University Criteria', 'Contextual Understanding', 'Semantic Relevance']
                },
                'improvement_metrics': {
                    'score_difference': round(semantic_enhancement, 2),
                    'accuracy_improvement': round(abs(semantic_enhancement) / max(university_total, 1) * 100, 2),
                    'method_advantages': [
                        'Standardized university criteria',
