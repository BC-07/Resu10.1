...
class PDSAssessmentApp:
    ...
    def get_candidates(self):
        ...
        try:
            ...
            for candidate in candidates:
                ...
                enhanced_score = self._calculate_enhanced_assessment_score(candidate, original_job_id)
    ...
    def get_assessment_trends(self):
        """Get assessment trends data over time"""
        try:
            days = request.args.get('days', 30, type=int)
            
            # Get trend data from database using the existing candidates table
            with db_manager.get_connection() as conn:
                cursor = conn.cursor()
                
                # Assessment trends over time - PostgreSQL compatible
                cursor.execute("""
                    SELECT 
                ...
                trends_data = [dict(row) for row in cursor.fetchall()]
                
                # Process types trends
                cursor.execute("""
                    SELECT 
                        COALESCE(processing_type, 'unknown') as processing_type,
                        COUNT(*) as count,
                        AVG(CASE WHEN score > 0 THEN score ELSE NULL END) as avg_score
                    FROM candidates
                    WHERE created_at >= NOW() - INTERVAL '%s days'
                ...
                processing_trends = [dict(row) for row in cursor.fetchall()]
                
                return jsonify({
                    'success': True,
                    'trends': {
                        'daily_assessments': trends_data,
                        'processing_type_trends': processing_trends,
                        'period_days': days,
                        'last_updated': datetime.now().isoformat()
                    }
                })
    ...
    def get_assessment_insights(self):
        ...
        try:
            ...
            # Get insights data using existing candidates table
            with db_manager.get_connection() as conn:
                cursor = conn.cursor()
                ...
                cursor.execute("""
                    SELECT 
                        ...
                        COUNT(*) as total_candidates,
                        AVG(CASE WHEN score > 0 THEN score ELSE NULL END) as avg_score,
                        COUNT(CASE WHEN status = 'shortlisted' THEN 1 END) as success_count
                    FROM candidates
                    WHERE created_at >= NOW() - INTERVAL '%s days'
                    GROUP BY category
                    HAVING COUNT(*) > 0
                ...
                category_insights = [dict(row) for row in cursor.fetchall()]
                ...
                cursor.execute("""
                    SELECT 
                        ...
                        COUNT(*) as count
                    FROM candidates
                    WHERE created_at >= NOW() - INTERVAL '%s days' AND score > 0
                    GROUP BY quality_level
                    ORDER BY MIN(score) DESC
                """, (days,))
                
                quality_insights = [dict(row) for row in cursor.fetchall()]
                
                # Generate insights safely
                total_candidates = sum(cat.get('total_candidates', 0) for cat in category_insights)
                avg_scores = [cat.get('avg_score') for cat in category_insights if cat.get('avg_score') is not None]
                ...
                insights = {
                    'performance_summary': {
                        ...
                        'top_performing_category': category_insights[0].get('category', 'N/A') if category_insights else 'N/A',
                        'period_days': days
                    },
                    'category_performance': category_insights,
                    'quality_distribution': quality_insights,
                    'recommendations': [
