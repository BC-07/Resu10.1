            ...
            with db_manager.get_connection() as conn:
                ...
                cursor.execute("""
                    SELECT 
                        ...
                        COUNT(*) as count
                    FROM candidates
                    WHERE created_at >= NOW() - INTERVAL %s DAY AND score > 0
                ...
                # Generate insights
                total_candidates = sum(cat['total_candidates'] for cat in category_insights)
                avg_overall_score = sum(cat['avg_score'] or 0 for cat in category_insights) / max(len(category_insights), 1)
                
                insights = {
                    'performance_summary': {
                        'total_candidates': total_candidates,
                        'avg_overall_score': round(avg_overall_score, 1),
                        'top_performing_category': category_insights[0]['category'] if category_insights else 'N/A',
                        'period_days': days
                    },
