...
def test_real_pds_hybrid():
    ...
    pds_extractor = ImprovedPDSExtractor()
    assessment_engine = EnhancedUniversityAssessmentEngine()
    ...
    job_posting = {
        ...
        'requirements': 'Teaching experience in education, Masters degree in Education or related field, government eligibility required, curriculum development experience preferred',
        'description': 'Responsible for teaching undergraduate and graduate courses in education, conducting research, curriculum development, and student guidance.',
        'qualifications': 'Masters degree in Education, Teaching experience, Research background, Government eligibility'
    }
    ...
    # Get all Excel files from SamplePDSFiles (they are .xlsx, not .pdf)
    excel_files = [f for f in os.listdir(sample_dir) if f.lower().endswith('.xlsx')]
    if not excel_files:
        print(f"❌ No Excel files found in {sample_dir}")
        return
    
    print(f"📁 Found {len(excel_files)} PDS files to test")
    print()
    
    # Test with all Excel files
    test_files = excel_files
    
    for i, excel_file in enumerate(test_files, 1):
        excel_path = os.path.join(sample_dir, excel_file)
        print(f"👤 Testing Candidate {i}: {excel_file}")
        print("=" * 50)
        
        try:
            # Extract PDS data (the method handles both PDF and Excel)
            print("📄 Extracting PDS data...")
            pds_data = pds_extractor.extract_pds_data(excel_path)
            ...
            university_result = assessment_engine._calculate_university_criteria_score(pds_data, job_posting)
            university_score = university_result.get('total_score', 0) if isinstance(university_result, dict) else university_result
            
            # Break down university scoring
            edu_score = assessment_engine._calculate_university_education_score(pds_data)
            exp_score = assessment_engine._calculate_university_experience_score(pds_data)
            training_score = assessment_engine._calculate_university_training_score(pds_data)
            eligibility_score = assessment_engine._calculate_university_eligibility_score(pds_data)
