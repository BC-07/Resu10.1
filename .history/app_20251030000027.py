...
class PDSAssessmentApp:
    ...
    def get_candidate_assessment(self, candidate_id):
        ...
        try:
            ...
            semantic_scores = self.semantic_engine.calculate_detailed_semantic_score(pds_data, job_posting)
    ...
    def get_candidate_assessment_for_job(self, candidate_id, job_id):
        ...
        try:
            ...
            try:
                ...
                university_scores = self.university_assessment_engine.calculate_detailed_scores(pds_data)
                university_total = self.university_assessment_engine.calculate_total_score(pds_data)
                
                # Semantic analysis
                semantic_result = self.semantic_engine.calculate_detailed_semantic_score(pds_data, job_posting)
                ...
                assessment_result = {
                    ...
                    'university_assessment': {
                        ...
                        'detailed_scores': {
                            ...
                            'performance': round(university_scores.get('performance', 0), 2),
                            'potential': round(university_scores.get('potential', 0), 2)
                        },
                        'breakdown': university_scores.get('breakdown', {})
                    },
                    
                    # Semantic analysis
                    'semantic_analysis': {
                        'overall_score': round(semantic_result.get('overall_score', 0) * 100, 2),
                        'education_relevance': round(semantic_result.get('education_score', 0) * 100, 2),
                        'experience_relevance': round(semantic_result.get('experience_score', 0) * 100, 2),
                        'skills_relevance': round(semantic_result.get('skills_score', 0) * 100, 2),
                        'training_relevance': round(semantic_result.get('training_score', 0) * 100, 2),
                    ...
                    'hybrid_score': round((university_total + (semantic_result.get('overall_score', 0) * 100)) / 2, 2)
                }
                
                logger.info(f"✅ Hybrid assessment completed - University: {university_total:.2f}, Semantic: {semantic_result.get('overall_score', 0) * 100:.2f}")
    ...
    def get_assessment_comparison_data(self, candidate_id):
        ...
        try:
            ...
            # Use default job posting for comparison
            default_job = {
                'title': 'Assistant Professor',
                'requirements': 'Masters degree, teaching experience, government eligibility',
            ...
            try:
                university_scores = {
                    'education': self.enhanced_assessment_engine._calculate_university_education_score(pds_data),
                    'experience': self.enhanced_assessment_engine._calculate_university_experience_score(pds_data),
                    'training': self.enhanced_assessment_engine._calculate_university_training_score(pds_data),
                    'eligibility': self.enhanced_assessment_engine._calculate_university_eligibility_score(pds_data)
                }
                logger.info("✅ University scores calculated")
            except Exception as e:
            ...
            try:
                semantic_scores = self.semantic_engine.calculate_detailed_semantic_score(pds_data, default_job)
                logger.info("✅ Semantic scores calculated")
            except Exception as e:
                logger.error(f"❌ Error calculating semantic scores: {e}")
                semantic_scores = {'overall_score': 0}
            
            # Calculate comparison metrics
            university_total = sum(university_scores.values())
            semantic_enhancement = semantic_scores.get('overall_score', 0) * 10
            
            comparison_data = {
                'traditional_assessment': {
                    'total_score': university_total,
                    'education_score': university_scores.get('education', 0),
                    'experience_score': university_scores.get('experience', 0),
                    'training_score': university_scores.get('training', 0),
                    'eligibility_score': university_scores.get('eligibility', 0)
                },
                'enhanced_assessment': {
                    'total_score': university_total + semantic_enhancement,
                    'university_score': university_total,
                    'semantic_bonus': semantic_enhancement,
                    'ai_enhancement': semantic_scores.get('overall_score', 0)
                },
                'differences': {
                    'improvement': semantic_enhancement,
                    'improvements': [
                        'Enhanced contextual understanding of job requirements',
                        'Semantic analysis of qualifications relevance',
                        'AI-powered skills and experience matching'
                    ]
                }
            }
    ...
    def get_semantic_analysis(self, candidate_id, job_id):
        ...
        try:
            ...
            try:
                semantic_result = self.semantic_engine.calculate_detailed_semantic_score(pds_data, job_data)
                logger.info("✅ Semantic analysis completed")
                
                # Format the response
                analysis_data = {
                    'semantic_analysis': {
                        'overall_relevance_score': semantic_result.get('overall_score', 0) * 100,
                        'education_relevance': semantic_result.get('education_score', 0) * 100,
                        'experience_relevance': semantic_result.get('experience_score', 0) * 100,
                        'skills_relevance': semantic_result.get('skills_score', 0) * 100,
                        'training_relevance': semantic_result.get('training_score', 0) * 100,
                        ...
                        'skills_insights': semantic_result.get('skills_insights', 'Skills profile matches well with job requirements.')
                    },
                    'insights': [
